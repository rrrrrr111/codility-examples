...
class Solution4Flags {
    public int solution(int[] A) {
        ...
        while (range > 1) {
            ...
            System.out.printf("Check for maxFlags:%s, range:%s\n", maxFlags, range);
                ...
                if (peaksCount < 1) {
                    ...
                    for (int j = startIndex; j <= stopIndex; j++) {
                        ...
                        if (range <= (dist[j] + prevDist)) {
                            prevDist = 0;
                            flags++;
                            System.out.printf("Check [%s;%s], peaks:%s, peak:%s counted, prevDist:%s, range:%s, flags:%s\n", startIndex, stopIndex, peaksCount, peak, prevDist, range, flags);
                        } else {
                            prevDist += dist[j];
                            System.out.printf("Check [%s;%s], peaks:%s, peak:%s ignored, prevDist:%s, range:%s, flags:%s\n", startIndex, stopIndex, peaksCount, peak, prevDist, range, flags);
                        }
                        if (peaksCount == peak) {
                            break;
                        }
